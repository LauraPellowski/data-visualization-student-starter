...
const highlightLine = (
  ...
  selection.selectAll('.data-point').style('opacity', 0.2);

  // Highlight points for this line
  selection.selectAll(`[data-line="${line}"]`).attr('r', 6).style('opacity', 1);

  // Highlight legend circle
  selection.select(`[data-legend-line="${line}"]`).select('circle').attr('r', 9);
};

const resetHighlight = (selection: Selection<SVGSVGElement, unknown, null, undefined>) => {
  // Restore all data points
  selection.selectAll('.data-point').attr('r', RADIUS).style('opacity', 1);

  // Restore all legend circles
  selection.selectAll('.legend-item circle').attr('r', 6);
};
...
export function renderCircles(
  ...
  const { data, xScale, yScale, xValue, yValue, width, height, margins } = options;
